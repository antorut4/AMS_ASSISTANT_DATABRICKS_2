...
CONFIGURAZIONE: non è più un dizionario dentro questo file. Il catalogo delle
impostazioni sta nel pacchetto amsconfig/; ciò che si modifica dal pannello
Impostazioni (l'ingranaggio in alto a destra) finisce nella tabella app_config,
dove sopravvive ai riavvii. Le uniche tre chiavi che non possono stare lì — schema,
warehouse e host, che servono per raggiungerla — si danno da variabile d'ambiente
(app.yaml o App Settings). Ogni altra chiave resta comunque sovrascrivibile da
APP__SEZIONE__CHIAVE, che vince su tutto.
...
from amsconfig import (APP_CONFIG_DDL, SqlStorage, apply_manifest_env,
                       available_seeds, build_service, seed_path)

logging.basicConfig(level=logging.INFO)
logger = logging.getLogger("ams")

# ###########################################################################
# #  CONFIGURAZIONE — non più un dizionario dentro questo file.              #
# #  Che cosa esiste, come si chiama e cosa significa: amsconfig/catalog.py  #
# #  Dove si trova QUESTA installazione:      variabili d'ambiente (app.yaml) #
# #  Le modifiche fatte dal pannello Impostazioni:      tabella app_config   #
# #  Come si legge tutto questo: la funzione `cfg` poco più sotto.           #
# ###########################################################################
# Un solo app.py per tutti i progetti, e nessun file di configurazione per
# progetto. Le uniche tre chiavi che NON possono stare nella tabella — perché
# servono per raggiungerla — si danno dall'ambiente, in app.yaml o nelle App
# Settings del workspace:
#     APP__STORAGE__SCHEMA        schema che contiene le tabelle dell'app
#     APP__COMPUTE__WAREHOUSE_ID  SQL Warehouse che esegue le query
#     APP__COMPUTE__DEFAULT_HOST  host del workspace (facoltativo: di norma
#                                 arriva già da DATABRICKS_HOST)
# Se mancano, l'App si ferma all'avvio dicendo quali impostare: vedi
# _check_bootstrap. Tutto il resto si configura dal pannello Impostazioni.
...
# --- accesso config -----------------------------------------------------------
# Il servizio risolve ogni chiave in quest'ordine: default applicativo, default
# dell'ambiente, valore salvato in app_config, variabile d'ambiente
# APP__SEZIONE__CHIAVE. L'ambiente vince su tutto di proposito: è la via di fuga
# quando un valore salvato dal pannello rende l'App inutilizzabile.
# La tabella viene collegata più sotto, appena `run_sql` esiste: fino a quel
# punto — e ogni volta che il warehouse non risponde — valgono default ed env.
# I valori dichiarati nell'app.yaml valgono anche FUORI dall'App. Nel runtime
# delle Databricks Apps li inietta la piattaforma, ma in un notebook — dove si
# esegue il bootstrap — nessuno legge quel file: senza questa riga bisognerebbe
# ripetere le stesse variabili a mano in ogni notebook. Quelle già presenti
# nell'ambiente non vengono toccate, quindi nell'App vince sempre la piattaforma.
_DA_MANIFEST = apply_manifest_env(
    os.path.join(os.path.dirname(os.path.abspath(__file__)), "app.yaml"))

config_service = build_service(
    logger=logger, actor=os.environ.get("DATABRICKS_APP_USER", "operator"))


def cfg(path: str, default=None):
    """Invariata per chi la usa: cambia soltanto da dove legge."""
    return config_service.get_config_value(path, default)


# Una riga nei log all'avvio: con lo stesso app.py su più ambienti, sapere su
# quale schema sta scrivendo è la prima cosa da guardare quando qualcosa non
# torna. Lo schema si legge senza toccare il database.
logger.info(
    f"schema attivo: "
    f"{config_service.bootstrap_value('storage.schema') or '(NON IMPOSTATO)'} · "
    f"warehouse {config_service.bootstrap_value('compute.warehouse_id') or '(NON IMPOSTATO)'}"
    + (f" · da app.yaml: {', '.join(_DA_MANIFEST)}" if _DA_MANIFEST else "")
    + ("" if config_service.bootstrap_value("storage.schema") else
       " — dichiara APP__STORAGE__SCHEMA e APP__COMPUTE__WAREHOUSE_ID nel blocco "
       "env: dell'app.yaml, oppure nelle App Settings del workspace"))


def _check_bootstrap() -> None:
    """Ferma l'avvio se manca l'indirizzo del database.

    Senza schema, `table()` produrrebbe '.tickets' e ogni query fallirebbe con un
    errore SQL incomprensibile; senza warehouse la connessione non parte proprio.
    Meglio un messaggio che dice cosa impostare, subito, che dieci minuti a
    leggere stack trace."""
    mancanti = [k for k in ("storage.schema", "compute.warehouse_id")
                if not str(cfg(k) or "").strip()]
    if not mancanti:
        return
    env = ", ".join("APP__" + k.upper().replace(".", "__") for k in mancanti)
    raise RuntimeError(
        f"Configurazione incompleta: manca {', '.join(mancanti)}. Imposta "
        f"{env} nell'app.yaml (blocco env:) o nelle App Settings del workspace. "
        f"Sono le uniche chiavi che non possono stare nella tabella delle "
        f"impostazioni, perché servono per raggiungerla.")
...
# Le impostazioni salvate dal pannello diventano leggibili DA QUI: prima non lo
# erano perché `sql_str` non esisteva ancora, e lo storage la riceve come
# riferimento, non per nome. Lo schema invece arriva sempre da default/env, mai
# dal database: è quello che stiamo per raggiungere.
config_service.attach_storage(SqlStorage(
    run_sql=run_sql, exec_sql=exec_sql, sql_str=sql_str,
    schema_provider=lambda: cfg("storage.schema"), logger=logger))
...
# La tabella delle impostazioni la definisce il modulo che ci scrive, così
# schema e codice restano nello stesso posto.
DDL += APP_CONFIG_DDL.replace("{schema}", "{s}") + ";"
...
def config_cli(args: list) -> str:
    """Impostazioni da riga di comando: per un Job Databricks o un terminale in
    cui il connettore SQL sia disponibile. In un notebook NON funziona (il
    runtime oscura il connettore): da lì si usa il pannello Impostazioni, che
    ha lo stesso «Importa JSON».

      python app.py config seed NOME      applica amsconfig/seeds/NOME.json
      python app.py config import FILE    applica un export JSON
      python app.py config export [FILE]  salva (o stampa) ciò che è cambiato
      python app.py config list           elenca gli scostamenti dai default
    """
    cmd = (args[0] if args else "").lower()
    if cmd in ("seed", "import", "export", "list"):
        _check_bootstrap()      # tutte queste toccano la tabella dell'ambiente
    if cmd == "seed":
        disponibili = available_seeds()
        nome = args[1] if len(args) > 1 else ""
        if not nome:
            return (f"Indica quale seme applicare: python app.py config seed "
                    f"<nome>. Disponibili: {', '.join(disponibili) or 'nessuno'}.")
        p = seed_path(nome)
        if not p:
            return (f"Seme '{nome}' inesistente. "
                    f"Disponibili: {', '.join(disponibili) or 'nessuno'}.")
        args, cmd = ["import", p], "import"
    if cmd == "import":
        if len(args) < 2:
            return "Manca il file: python app.py config import <file.json>"
        with open(args[1], encoding="utf-8") as f:
            rep = config_service.import_config(f.read(), ACTOR)
        parti = [f"applicate {len(rep['saved'])}",
                 f"già uguali {len(rep['unchanged'])}"]
        if rep.get("ignored"):
            parti.append(f"chiavi ignote {len(rep['ignored'])}")
        if rep["env"]:
            parti.append(f"forzate da env: {', '.join(rep['env'])}")
        if rep["errors"]:
            parti.append("ERRORI -> " + "; ".join(f"{k}: {v}"
                                                  for k, v in rep["errors"].items()))
        return f"[{config_service.label}] " + " · ".join(parti)
    if cmd == "export":
        testo = config_service.export_config()
        if len(args) > 1:
            with open(args[1], "w", encoding="utf-8") as f:
                f.write(testo)
            return f"[{config_service.label}] scritto {args[1]}"
        return testo
    if cmd == "list":
        righe = [f"{i.key:44} = "
                 f"{json.dumps(i.current_value, ensure_ascii=False)[:70]}  [{i.source}]"
                 for i in config_service.get_config() if i.source != "default"]
        return ("\n".join(righe) if righe else
                f"[{config_service.label}] nessuna impostazione personalizzata: "
                f"tutto sui valori di default.")
    return config_cli.__doc__
...
T = {
    # Senza database: questo dizionario si costruisce quando si IMPORTA il
    # modulo, e importarlo non deve costare una query al warehouse (in notebook
    # il connettore SQL è oscurato dal runtime e fallirebbe sempre). Il colore
    # eventualmente cambiato dal pannello lo rilegge _refresh_design_tokens()
    # quando si costruisce la pagina, che è l'unico momento in cui serve.
    "brand": config_service.bootstrap_value("branding.accent_color", "#EC008C"),
...
def _refresh_design_tokens():
    """I token finiscono nel tema e nel CSS, che vengono generati UNA volta sola
    quando si costruisce la pagina. Rileggerli qui è ciò che rende visibile un
    colore cambiato dal pannello Impostazioni: senza, servirebbe riavviare."""
    T["brand"] = cfg("branding.accent_color", T["brand"])
...
def build_app():
    import gradio as gr
    from amsconfig import ui as config_ui
    ...
    with gr.Blocks(title=cfg("branding.header_title"), theme=_theme(), css=_css() + config_ui.CSS,
        ...
        with gr.Column(elem_id="ams-tabwrap"):
            # L'ingranaggio è dichiarato PRIMA delle tab: in Gradio 4 non si
            # può inserire un pulsante DENTRO la barra delle tab, che genera il
            # componente. Il CSS lo porta sulla stessa riga, tutto a destra.
            gear = gr.Button("⚙", elem_id="ams-gear", scale=0)
            with gr.Tabs():
        ...
        # Pannello Impostazioni: un drawer che il CSS fissa a destra, aperto
        # dall'ingranaggio. Sta fuori dalle tab di proposito — è un livello
        # sopra la navigazione, non una sua voce.
        _settings_drawer, _settings_open = config_ui.mount(
            gr, config_service, _ok, _err, lambda: ACTOR)
        gear.click(*_settings_open)
...
def main():
    ...
    _check_bootstrap()
    # Le impostazioni salvate vanno lette PRIMA di costruire la pagina: titolo,
    # elenco del team e colori entrano nella UI una volta sola. Se il warehouse
    # non risponde si prosegue sui default invece di non partire.
    if not config_service.warm():
        logger.warning("impostazioni non lette dal database: uso default ed env")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "bootstrap":
        ...
        # python app.py config seed | import FILE | export [FILE] | list
        print(config_cli(sys.argv[2:]))
